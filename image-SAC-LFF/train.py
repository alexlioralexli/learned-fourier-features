...
def parse_args():
    ...
    # LFF parameters
    parser.add_argument("--fourier_dim", default=32, type=int)
    parser.add_argument("--sigma", default=1.0, type=float)
    parser.add_argument("--fourier_mult", default=2, type=float)
...
def evaluate(env, agent, video, num_episodes, L, step, args):
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                # center crop image
                if args.encoder_type in {'pixel', 'fourier_pixel', 'fair_pixel'} and 'crop' in args.data_augs:
            ...
            # logger.record_tabular('eval/' + prefix + 'episode_reward', episode_reward)
            all_ep_rewards.append(episode_reward)
        
        # L.log('eval/' + prefix + 'eval_time', time.time()-start_time , step)
        logger.record_tabular('eval/' + prefix + 'eval_time', time.time()-start_time)
        mean_ep_reward = np.mean(all_ep_rewards)
        best_ep_reward = np.max(all_ep_rewards)
        std_ep_reward = np.std(all_ep_rewards)
        # L.log('eval/' + prefix + 'mean_episode_reward', mean_ep_reward, step)
        # L.log('eval/' + prefix + 'best_episode_reward', best_ep_reward, step)
        logger.record_tabular('eval/' + prefix + 'mean_episode_reward', mean_ep_reward)
        logger.record_tabular('Eval returns', mean_ep_reward)
        logger.record_tabular('eval/' + prefix + 'best_episode_reward', best_ep_reward)
...
def make_agent(obs_shape, action_shape, args, device):
    if args.encoder_type in {'fourier_pixel', 'fair_pixel'}:
        encoder_kwargs = dict(sigma=args.sigma, fourier_dim=args.fourier_dim, train_B=args.train_B,
                              concatenate_fourier=args.concatenate_fourier, fourier_mult=args.fourier_mult)
    else:
        encoder_kwargs = dict()
...
def main():
    ...
    env = dmc2gym.make(
        ...
        from_pixels=(args.encoder_type in {'pixel', 'fourier_pixel', 'fair_pixel'}),
    ...
    if args.encoder_type in {'pixel', 'fourier_pixel', 'fair_pixel'}:
        env = utils.FrameStack(env, k=args.frame_stack)
    
    # make directory
    ts = time.gmtime() 
    ts = time.strftime("%m-%d-%Y-%H-%M-%S", ts)
    ...
    # begin added
    args.env = f'dm.{args.domain_name}.{args.task_name}'
    log_dir = create_env_folder(args.env, args.expID, 'SAC', args.encoder_type, test=args.test)
    ...
    for step in range(args.num_train_steps):
        ...
        if step % args.eval_freq == 0:
            ...
            logger.record_tabular('Time since last eval (s)', time.time() - last_time)
            last_time = time.time()
